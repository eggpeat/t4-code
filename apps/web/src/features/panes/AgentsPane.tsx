...
function ElapsedLabel({ startedAt, live }: { readonly startedAt: string | null; readonly live: boolean }) {
  const nowMs = useNowTick(live);
  const label = formatElapsed(startedAt, nowMs);
  if (label === "") return null;
  return <span className="shrink-0 font-mono text-muted-foreground text-xs tabular-nums">{label}</span>;
}
...
const AgentRow = memo(function AgentRow({ api, id, depth, selected, focused, onSelect }: AgentRowProps) {
  ...
  return (
    <div
      ...
      className={cn(
        "flex cursor-pointer flex-col gap-0.5 rounded-md pe-2 py-1.5 outline-none transition-colors duration-(--motion-duration-fast)",
    ...
    >
      ...
      {(node.currentTool !== null || node.progress !== null || (node.contextUsed !== null && node.contextLimit !== null)) && (
      ...
      {node.evidence !== null && node.state !== "aborted" && node.state !== "failed" && (
        <p className="ps-3.5 text-muted-foreground text-xs">{node.evidence}</p>
      )}
    </div>
  );
});
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-3 pb-2">
          ...
          <dl className="flex flex-wrap gap-x-4 gap-y-0.5 pb-1.5 text-xs">
            {facts.map((fact) => (
              <div className="flex min-w-0 items-baseline gap-1.5" key={fact.label}>
                ...
                <dd className={cn("truncate", fact.mono && "font-mono")}>{fact.value}</dd>
              </div>
            ))}
          </dl>
        )}
        ...
        {node.transcript.length > 0 ? (
          <ol aria-label="Agent transcript (read-only)" className="flex flex-col gap-1.5">
            {node.transcript.map((entry) => (
              <li className="rounded-md bg-secondary/60 px-2 py-1.5" key={entry.id}>
                ...
                <span className={cn("block text-xs", entry.role === "tool" && "font-mono")}>
                  {entry.text}
                </span>
              </li>
            ))}
          ...
          <p className="py-1 text-muted-foreground text-xs">
            No conversation recorded for this agent yet.
          </p>
        )}
      </div>
      <div className="flex items-center gap-1.5 border-border border-t px-3 py-2">
        <Button
          disabled={(node.state !== "running" && node.state !== "waiting") || !actions.agentSteer.enabled}
        ...
        <Button
          disabled={(node.state !== "parked" && node.state !== "idle") || !actions.agentWake.enabled}
        ...
        <span className="text-muted-foreground text-xs">Read-only transcript</span>
      </div>
    </section>
  );
}
...
export function AgentsPane({
  ...
  const rows = useMemo(
    () => buildAgentTreeRows(api.getState().agentMap),
    [api, structure],
  );
  ...
  const onSelect = useCallback((id: string) => api.getState().selectAgent(id), [api]);
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      ...
      {selectedId !== null && <AgentDetail api={api} id={selectedId} sessionId={sessionId} />}
