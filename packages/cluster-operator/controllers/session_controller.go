...
const DefaultInternalAuthSecret = "t4-cluster-internal-auth"
...
func (r *SessionReconciler) Reconcile(ctx context.Context, request ctrl.Request) (ctrl.Result, error) {
	...
	if !session.DeletionTimestamp.IsZero() { return r.reconcileDelete(ctx, &session) }
	if controllerutil.AddFinalizer(&session, clusterv1alpha1.SessionFinalizer) {
		if err := r.Update(ctx, &session); err != nil { return ctrl.Result{}, err }
	}
	if r.RuntimeImage == "" {
	...
	if err := r.Get(ctx, types.NamespacedName{Namespace: session.Namespace, Name: session.Spec.HostRef}, &host); err != nil {
		if apierrors.IsNotFound(err) { return ctrl.Result{RequeueAfter: 30 * time.Second}, r.updateSessionFailure(ctx, &session, "HostReady", "HostNotFound", "referenced T4ClusterHost does not exist") }
	...
	if err := r.Get(ctx, types.NamespacedName{Namespace: session.Namespace, Name: session.Spec.WorkspaceRef}, &workspace); err != nil {
		if apierrors.IsNotFound(err) { return ctrl.Result{RequeueAfter: 30 * time.Second}, r.updateSessionFailure(ctx, &session, "WorkspaceReady", "WorkspaceNotFound", "referenced T4Workspace does not exist") }
		return ctrl.Result{}, err
	}
	...
	if err := r.Get(ctx, types.NamespacedName{Namespace: session.Namespace, Name: workspace.Status.PVCName}, &pvc); err != nil {
		if apierrors.IsNotFound(err) { return ctrl.Result{RequeueAfter: 5 * time.Second}, r.updateSessionFailure(ctx, &session, "WorkspaceReady", "PVCNotFound", "workspace PVC does not exist") }
		return ctrl.Result{}, err
	}
	...
	if err := r.Get(ctx, types.NamespacedName{Namespace: session.Namespace, Name: serviceName}, &service); apierrors.IsNotFound(err) {
		...
		if err := controllerutil.SetControllerReference(&session, &service, r.Scheme); err != nil { return ctrl.Result{}, err }
		if err := r.Create(ctx, &service); err != nil && !apierrors.IsAlreadyExists(err) { return ctrl.Result{}, err }
	} else if err != nil { return ctrl.Result{}, err }
	...
	if err := r.Get(ctx, types.NamespacedName{Namespace: session.Namespace, Name: podName}, &pod); apierrors.IsNotFound(err) {
		...
		if err := controllerutil.SetControllerReference(&session, &pod, r.Scheme); err != nil { return ctrl.Result{}, err }
		if err := r.Create(ctx, &pod); err != nil && !apierrors.IsAlreadyExists(err) { return ctrl.Result{}, err }
	} else if err != nil { return ctrl.Result{}, err }

	original := session.Status
	if session.Status.Conditions != nil { original.Conditions = append([]metav1.Condition(nil), session.Status.Conditions...) }
	...
	if !reflect.DeepEqual(original, session.Status) {
		if err := r.Status().Update(ctx, &session); err != nil { return ctrl.Result{}, err }
	}
	if !podReady(&pod) { return ctrl.Result{RequeueAfter: 5 * time.Second}, nil }
...
func (r *SessionReconciler) desiredPod(session *clusterv1alpha1.T4Session, pvcName, podName string, labels map[string]string) corev1.Pod {
	...
	if shmSize.IsZero() { shmSize = apiresource.MustParse("1Gi") }
	temporarySize := r.TemporarySize.DeepCopy()
	if temporarySize.IsZero() { temporarySize = apiresource.MustParse("2Gi") }
	...
	internalSecret := r.InternalAuthSecret
	if internalSecret == "" { internalSecret = DefaultInternalAuthSecret }
	excluded := r.ExcludedNodeNames
	if len(excluded) == 0 { excluded = []string{"k3s-worker-02"} }
	...
	container := corev1.Container{
		...
		Env: []corev1.EnvVar{
			{Name: "T4_CLUSTER_INTERNAL_TOKEN", ValueFrom: &corev1.EnvVarSource{SecretKeyRef: &corev1.SecretKeySelector{LocalObjectReference: corev1.LocalObjectReference{Name: internalSecret}, Key: "token"}}},
...
func (r *SessionReconciler) reconcileDelete(ctx context.Context, session *clusterv1alpha1.T4Session) (ctrl.Result, error) {
	if !controllerutil.ContainsFinalizer(session, clusterv1alpha1.SessionFinalizer) { return ctrl.Result{}, nil }
	originalStatus := session.Status
	if session.Status.Conditions != nil { originalStatus.Conditions = append([]metav1.Condition(nil), session.Status.Conditions...) }
	session.Status.ObservedGeneration = session.Generation
	...
	for _, object := range []client.Object{
		...
		if err == nil {
			...
			if object.GetDeletionTimestamp().IsZero() {
				if err := r.Delete(ctx, object); err != nil && !apierrors.IsNotFound(err) { return ctrl.Result{}, err }
			}
		} else if !apierrors.IsNotFound(err) { return ctrl.Result{}, err }
	}
	if remaining { return ctrl.Result{RequeueAfter: time.Second}, nil }
...
func (r *SessionReconciler) updateSessionFailure(ctx context.Context, session *clusterv1alpha1.T4Session, conditionType, reason, message string) error {
	...
	if reflect.DeepEqual(original, session.Status) { return nil }
...
func podReady(pod *corev1.Pod) bool {
	for _, item := range pod.Status.Conditions {
		if item.Type == corev1.PodReady { return item.Status == corev1.ConditionTrue }
	}
	return false
}
